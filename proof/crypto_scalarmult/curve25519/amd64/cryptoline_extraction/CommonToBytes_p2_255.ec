...
proof.
    proc. auto => />. 
    move => H H0 H1. do split. by rewrite H1. move => H2. 
    do split. by rewrite H2. move => H3 H4.
    do split. rewrite !H1 !H2. 
    ...
    + have ->: 18446744073709551616 <= u64i _f.[0] + 19 = true. 
    + move: H H0.     
    + rewrite ultE uleE E of_uintK pmod_small //=.
    ...
    + rewrite !valRep4E !/val_digits !/to_list !/mkseq -!iotaredE => />.
    + move: W64.to_uint_cmp. smt().
    + rewrite !b2i1 => />. rewrite to_uint_shr //=. rewrite to_uintD //=.
    rewrite to_uintD_small //=. 
    + move: H H0.     
    + rewrite ultE uleE E of_uintK pmod_small //=.
    + rewrite !valRep4E !/val_digits !/to_list !/mkseq -!iotaredE => />.
    + move: W64.to_uint_cmp. smt().    
    have ->: (u64i _f.[3] + 1 + (u64i _f.[3] + 1)) %% 18446744073709551616 = (u64i _f.[3] + 1 + (u64i _f.[3] + 1))- 18446744073709551616. 
    + move: H H0.     
    + rewrite ultE uleE E of_uintK pmod_small //=.
    + rewrite !valRep4E !/val_digits !/to_list !/mkseq -!iotaredE => />.
    + move: W64.to_uint_cmp. smt(). smt().
    move => H5. by rewrite H5.       
qed.
