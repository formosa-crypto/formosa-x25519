...
lemma eq_ph_add_and_double (qx : zp) (nqs : (zp * zp) * (zp * zp)):
    phoare [CurveProcedures.add_and_double :
        init = qx /\
        x2 = nqs.`1.`1 /\
        z2 = nqs.`1.`2 /\
        x3 = nqs.`2.`1 /\
...
lemma eq_ph_montgomery_ladder_step (k : W256.t) (init : zp) (nqs : (zp * zp) * (zp * zp) * bool) (ctr : int) :
    phoare [CurveProcedures.montgomery_ladder_step :
        k' = k           /\
        init' = init     /\
        x2 = nqs.`1.`1   /\
        z2 = nqs.`1.`2   /\
...
lemma eq_ph_montgomery_ladder (init : zp) (k : W256.t) :
  phoare [CurveProcedures.montgomery_ladder :
        init' = init /\
        k.[0] = false /\
        k' = k
        ==>
...
lemma eq_ph_invert (z : zp) :
    phoare[CurveProcedures.invert :
        fs =  z
        ==>
        res = op_invert2 z] = 1%r.
proof.
    by conseq ill_invert (eq_proc_op_invert z).
qed.
...
lemma eq_ph_encode_point (q : zp * zp) :
    phoare[CurveProcedures.encode_point :
        x2 =  q.`1 /\ z2 = q.`2
        ==>
        res = op_encode_point q] = 1%r.
proof.
    by conseq ill_encode_point (eq_proc_op_encode_point q).
qed.
